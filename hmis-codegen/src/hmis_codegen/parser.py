            ...
            for method, operation_def in methods.items():
                if method in ['get', 'post', 'put', 'patch', 'delete']:
                    ...
                    effects = self._parse_effects(
                        operation_def.get('x-effects', []))

                    # Parse request schema
                    request_schema = None
                    if 'requestBody' in operation_def:
                        content = operation_def['requestBody'].get(
                            'content', {})
                        json_content = content.get('application/json', {})
                        schema_ref = json_content.get(
                            'schema', {}).get('$ref', '')
                        if schema_ref:
                    ...
                    for status_code, response_def in operation_def.get('responses', {}).items():
                        ...
                        schema_ref = json_content.get(
                            'schema', {}).get('$ref', '')
                        if schema_ref:
                            response_schemas[status_code] = schema_ref.split(
                                '/')[-1]
...
class FHIRMappingsParser:
    ...
    def parse(self) -> FHIRMappingsFile:
        ...
        for hmis_name, mapping_data in self.data.get('resource_mappings', {}).items():
            ...
            for field_name, field_data in mapping_data.get('field_mappings', {}).items():
                ...
                field_mappings[field_name] = FHIRFieldMapping(
                    ...
                    collection_consent=privacy_controls.get(
                        'collection_consent'),
                    retention_period=privacy_controls.get('retention_period'),
                    disclosure_restriction=privacy_controls.get(
                        'disclosure_restriction'),
                    encryption_required=privacy_controls.get(
                        'encryption_required'),
                    audit_trail_required=privacy_controls.get(
            ...
            resource_mappings[hmis_name] = FHIRResourceMapping(
                ...
                semantic_extensions=mapping_data.get(
                    'semantic_extensions', []),
        ...
        return FHIRMappingsFile(
            ...
            privacy_security_mappings=self.data.get(
